...
def scrape_repo(target_url, department, file):
    ...
    with open(file, "a", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, ["title", "authors", "department", "keywords", "title_url", "author_urls",
                                    "publishing_info"])
        writer.writeheader()
            ...
            for t in titles:
                ...
                if authors:
                    for a in authors:
                        ...
                        author_href = urljoin(base_url, link_tag["href"]) if link_tag and link_tag.get(
                            "href") else "N/A"
...
def scrape_employees(current_time):
